...
**XO1 (CL1A):** the audio subsystem is based on the '97 standard audio codec. The external microphone input is **monoaural (mono)** and it includes a programmable gain preamplifier. A DC blocking capacitor may be connected in software in order to measure audio signals or AC. To utilize this input as a sensor the capacitor needs to be disconnected. A polarizing 2v input voltage may be applied that is controlled by software. This is used to feed an external electret microphone or if desired it is possible to sense if an external switch is open or closed.
